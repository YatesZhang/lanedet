...
from lanepoints_conv import LanePointsConv2 as LanePointsConv
# from lanepoints_conv import LanePointsConv
from mmdetv2.models.builder import NECKS
import pdb 
...
class DeformFPN(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            def_conv = LanePointsConv(
                ...
                feat_channels=feat_channels,    # 256
                point_feat_channels=out_channels,    # 64
    ...
    # 半精度浮点格式（FP16）Float Precision使用16位浮点数加快训练和推理
    def forward(self, inputs):
        inputs = list(inputs)
        

        if len(inputs) > len(self.in_channels):    # False
            del inputs[0]
        # inputs: shape:[1, 128, 40, 100], [1, 256, 20, 50], [1, 512, 10,25] 
        
        if self.trans_cfg is not None:
            # self.trans_head: attention layer
            trans_feat = self.trans_head(inputs[self.trans_idx])    # trans_idx = -1
            if self.trans_mode == 'replace':
                inputs[-1] = trans_feat    # [1,512,10,25] -> [1,64,10,25]

        # build laterals
        # adjust the channels layer3,4,5
        # lateral_convs: [conv1x1(128, 64), conv1x1(256,64), conv1x1(64,64)]
        # deform_convs: type: LanePointsConv
        # self.deconv_layer: [True, False, False]
        laterals = []
        deform_points = []
        # [i.dcn_kernel for i in self.def_convs] = [7, 5, 3]
        for i, (lateral_conv, deform_conv) in enumerate(zip(self.lateral_convs, self.def_convs)):
            if self.deconv_layer[i] and self.deconv_before:    # False
                mid_feat = lateral_conv(inputs[i + self.start_level])    # start_level: 0
                d_feat, points = deform_conv(mid_feat)
                if self.use_res:    # use_res == False
        ...
        for i in range(used_backbone_levels - 1, 0, -1):    # i: [2, 1]
            ...
            if i-1 == 0:
                aux_feat = laterals[0]    # aux_feat.shape: [1, 64, 40, 100]
            # self.deconv_layer: [True, False, False]
            if self.deconv_layer[i-1] and not self.deconv_before:    # if i == 1
                if self.use_res:    # False
                    mid_feat, points = self.def_convs[i-1](laterals[i - 1])    # laterals[0]
                    laterals[i - 1] = mid_feat + laterals[i - 1]
                else:
                    laterals[i - 1], points = self.def_convs[i-1](laterals[i - 1])
                deform_points[i-1] = points    # points: [1, 14, 40, 100]

        # build outputs
        # part 1: from original levels
        # adjust the feature by conv
        # fpn_convs: [conv3x3(64, 64), conv3x3(64,64), conv3x3(64,64)]

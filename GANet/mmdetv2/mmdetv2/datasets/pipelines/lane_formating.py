...
def clamp_line(line, box, min_length=0):
    ...
    try:
        ...
        if isinstance(I, LineString):
            ...
            pts = []
            for Istring in I.geoms:    # list(I) is deprecated
...
class CollectLanePoints(Collect):
    ...
    def target(self, results):
        ...
        hm_h = int(output_h // self.hm_down_scale)  # 40
        hm_w = int(output_w // self.hm_down_scale)  # 100
        results['hm_shape'] = [hm_h, hm_w]          # [40, 100]
        results['mask_shape'] = [mask_h, mask_w]    # [40, 100]
        ...
        end_points = []
        start_points = []
        for l in range(self.fpn_layer_num):     # 3
            lane_points = []
            fpn_down_scale = self.fpn_down_scale[l]     # fpn_down_scale: [8, 16, 32]
            fn_h = int(output_h // fpn_down_scale)      # down scale from output: [320,800]
            ...
            for i, pts in enumerate(gt_points):  # per lane
                ...
                if pts is not None:
                    ...
                    pts = np.int32(pts_f)                   # column1: <= 99 colum2: <= 39
                    lane_points.append(pts[None, :, ::-1])  # [a_i,b_i] -> [b_i, a_i] -> column2: <= 99 colum1: <= 39
        ...
        # FPN use 3 down sample scale, respectively, results have updated 3 keys: lane_points_l{0,1,2}
        # lane_points_l0 : [40, 100]  (6, 41, 2)  type: int
        # lane_points_l1 : [20, 50]   (6, 21, 2)  type: int
        # lane_points_l2 : [10, 25]   (6, 11, 2)  type: int
        ...
        for pts in gt_points:  # per lane
            ...
            if pts is not None and len(pts) > 1:
                ...
                delta_idx = len(pts) // self.joint_nums         # joint_nums == 1
                end_points.append(end_point)
                start_points.append(start_point)
                for i in range(self.joint_nums):
                    joint_points.append(pts[i * delta_idx])     # start point as defualt when joint_nums==1
                ...
                for pt in pts:
                    ...
                    # reg = pt - pt_int
                    reg_x = pt[0] - pt_int[0]                # error between decimal and int
                    reg_y = pt[1] - pt_int[1]  
                    reg_hm[0, pt_int[1], pt_int[0]] = reg_x  # [2 H W]
                    reg_hm[1, pt_int[1], pt_int[0]] = reg_y  # [2 H W]
                    if abs(reg_x) < 2 and abs(reg_y) < 2:
                        offset_mask[0, pt_int[1], pt_int[0]] = 1  # mask where have points, offset_mask: [1, 40, 100]
        ...
        results['gt_cpts_hm'] = DC(to_tensor(gt_hm).float(), stack=True, pad_dims=None)         # heatmap: 4 start points
        results['gt_kpts_hm'] = DC(to_tensor(gt_kpts_hm).float(), stack=True, pad_dims=None)    # heatmap: key points
        # error between decimal and int :downsample offset
        ...
        # mask where have points, offset_mask: [1, 40, 100], which is indexed from pt_int
        results['offset_mask'] = DC(to_tensor(offset_mask).float(), stack=True,
                                    pad_dims=None)  # key points -> center point offset
        # assign weight for offset mask:
        results['offset_mask_weight'] = DC(to_tensor(offset_mask_weight).float(), stack=True,
                                           pad_dims=None)  # key points -> center point offset
        results['gt_vp_hm'] = DC(to_tensor(gt_vp_hm).float(), stack=True, pad_dims=None)        # np.zeroes
        results['gt_masks'] = gt_masks                                                          # gt_mask = []
...
class DrawLabels(Collect):
    def __init__(
        ...
        super(DrawLabels, self).__init__(keys, meta_keys)
        self.down_scale = down_scale
        self.fpn_layer_num = fpn_layer_num
        self.hm_down_scale = hm_down_scale if hm_down_scale is not None else down_scale
        self.max_mask_sample = max_mask_sample
    ...
    def target(self, results):
        ...
        # [1, 40, 100]
        gt_kpts_hm = np.zeros((1, hm_h, hm_w), np.float32)
        hyper_plane = np.zeros((1, hm_h, hm_w), np.float32)
        offset_mask_weight = np.zeros((1, hm_h, hm_w), np.float32)
        plane_height = 0
            ...
            if pts is not None and len(pts) > 1:
                ...
                for pt in pts:
                    ...
                    for i in range(self.joint_nums):
                        ...
                        # weight mask
                        mask_value = assign_weight(offset_y, max_y, self.joint_nums, self.joint_weights)
                        offset_mask_weight[0, pt_int[1], pt_int[0]] = mask_value

                    # TODO : key points -> root_i point offset for each lane
                    hyper_plane[0, pt_int[1], pt_int[0]] = plane_height 
            plane_height += self.dheight
            
        offset_mask = offset_mask_weight > 0    # shape: [1, 40, 100]
        offset_mask = np.concatenate([offset_mask, offset_mask], axis=0)    # shape: [2, 40, 100]
        
        results['gt_kpts_hm'] = DC(to_tensor(gt_kpts_hm).float(), stack=True, pad_dims=None)    # heatmap: key points
        # error between decimal and int :downsample offset
        results['int_offset'] = DC(to_tensor(reg_hm).float(), stack=True, pad_dims=None)  
        results['hyper_plane'] = DC(to_tensor(hyper_plane).float(), stack=True, pad_dims=None)

        results['offset_mask_weight'] = DC(to_tensor(offset_mask_weight).float(), stack=True, pad_dims=None)  
        results['offset_mask'] = DC(to_tensor(offset_mask).float(), stack=True, pad_dims=None)
        results['down_scale'] = self.down_scale
        results['hm_down_scale'] = self.hm_down_scale
        return True

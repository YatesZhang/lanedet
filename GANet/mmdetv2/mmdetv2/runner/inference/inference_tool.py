...
import torch 
import warnings 
from .converters import get_hm, get_offset, get_error, get_height
...
def lanes_throwing(_lanes: list, max_lanes=4):
    ...
    for _lane in _lanes: 
        if len(_lane) > 3:
            long_lanes.append(_lane)
    _lanes = long_lanes
    if len(_lanes) <= 4:
        return _lanes
    _lanes = [_lane for _lane in sorted(_lanes, key=lambda _lane: len(_lane), reverse=True)]
...
def inference(hm: np.ndarray, **kwargs):
    """_summary_
    ...
    lanes = [_lane for _lane in sorted(lanes, key=lambda _lane: len(_lane), reverse=True)]
...
class Vec3(object):
    ...
    def scale_up(self):
        self.x = self.x * 8 + 270    # 40 -> 320+270 = 590 
        self.y = self.y * 16.4       # 100 -> 1640
        return self 
        
def inference_hyper_plane(data: dict, hm_thr=0.3, radius=10):
    hm = get_hm(data=data)
    # error = get_error(data=data)
    hyper_plane = get_height(data=data)
    
    hm = hm > hm_thr    # inference mask
    index = np.where(hm)
    height = hyper_plane[index]
    
    # get points:
    points = []
    for i, (x, y) in enumerate(zip(index[0], index[1])):
        points.append(Vec3(x, y, height[i])) 
    if len(points) == 0:
        return points
    ...
    clusters = []
    # pdb.set_trace()
    points.sort(key=lambda vec3: vec3.x, reverse=True)   # sorted by row
    if points is None: 
        return []
    for vec3 in points:
        if len(clusters) == 0:
            clusters.append([vec3])
            continue
        
        FOUND = False 
        for i, _lane in enumerate(clusters): 
            end_point = _lane[-1]
            if vec3.distance(end_point) <= radius:
                clusters[i].append(vec3)
                FOUND = True 
                break 
        if not FOUND: 
            clusters.append([vec3])
    clusters = lanes_throwing(clusters, max_lanes=4)
    
    # scale up: 
    lanes = []
    for _lane in clusters: 
        lanes.append([vec3.scale_up() for vec3 in _lane])             
    return lanes 
    
    


def infer_from_offset_hm(data: dict):
    offset = get_offset(data=data)    # get offset map 
    hm = get_hm(data=data)
    error = get_error(data=data) 

...
def adjust_point(hm_points,
                 ...
                 img_shape,
                 tgt_shape=(590, 1640)):
    left, top, right, bot = crop_bbox    # (0, 270, 1640, 590)
    h_img, w_img = img_shape[:2]
    crop_width = right - left    # 1640
    crop_height = bot - top      # 590 - 270
    ratio_x = crop_width / w_img
    ratio_y = crop_height / h_img
    offset_x = (tgt_shape[1] - crop_width) / 2    # 0
    offset_y = top    # 270
...
def single_gpu_test(seg_model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            mkdir(dst_folder)
            # import pdb
            # pdb.set_trace() # and set thr = 0.18:
...
def main():
    ...
    # to summarize results with CULane evaluator:

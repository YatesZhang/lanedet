...
__all__ = ['GroupRandomCrop', 'GroupCenterCrop', 'GroupRandomPad', 'GroupCenterPad',
           'GroupRandomScale', 'GroupRandomHorizontalFlip', 'GroupNormalize','FastResizer']
...
class FastResizer(object):
    def __init__(self, img_size=(590,1640), target_size=(288,800)) -> None:
    ...
    def resize_lable(img,size=(288,800)):
        ...
        x = np.round(x * (src_w/w)).astype('int')
        y = np.round(y * (src_h/h)).astype('int')
        img = img[x,:]
        img = img[:,y]
        return img

    def __call__(self,img_group):
        assert len(img_group)==2 and len(img_group[0].shape)==3 and len(img_group[1].shape==2)
        
        # resize the img:
        img_group[0] = cv2.resize(img_group[0], self.size,
                              interpolation=cv2.INTER_CUBIC)
        img_group[1] = self.resize_lable(img_group[1])
        return img_group

...
def focal_loss(input: torch.Tensor,
               ...
               reduction: str = 'none',
               eps: float = 1e-8
               ,ignore_label=None) -> torch.Tensor:
    ...
    focal = -alpha * weight * torch.log(F.relu(input_soft) + 1e-6)
...
class FocalLoss(nn.Module):
    ...
    """
    def __init__(self, alpha: float, gamma: float = 2.0, reduction: str = 'none',ignore_label=None) -> None:
    ...
    def forward(  # type: ignore
        ...
        return focal_loss(input, target, self.alpha, self.gamma, self.reduction, self.eps, self.ignore_label)

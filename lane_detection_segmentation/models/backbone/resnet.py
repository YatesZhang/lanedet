...
class PSABlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if groups != 1 or base_width != 64:
            raise ValueError(
                'PSABlock only supports groups=1 and base_width=64')
        # if dilation > 1:
        #     raise NotImplementedError(
        #         "Dilation > 1 not supported in BasicBlock")
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv3x3(inplanes, planes, stride, dilation=dilation)
        self.att = PSA_s(inplanes=planes, planes=planes)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes, dilation=dilation)
        self.bn2 = norm_layer(planes)
    ...
    def __init__(self, cfg):
        ...
        if not self.psa:
            ...
            self.model = eval(cfg.resnet)(
                pretrained=cfg.pretrained,
                replace_stride_with_dilation=cfg.replace_stride_with_dilation
                , in_channels=self.in_channels,psa=True)
        self.out = None
        self.psa = cfg.psa
        if cfg.out_conv:
            out_channel = 512
...
def resnet18(pretrained=False, progress=True, **kwargs):
    ...
    """
    if 'psa' in kwargs and kwargs['psa']:
        kwargs.pop('psa')
        return _resnet('resnet18', PSABlock, [2, 2, 2, 2], pretrained, progress,
                    **kwargs)
    else:
